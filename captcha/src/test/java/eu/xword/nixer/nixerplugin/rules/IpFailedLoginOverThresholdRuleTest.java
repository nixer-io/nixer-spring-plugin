...
class IpFailedLoginOverThresholdRuleTest {
    ...
    @Mock
    private IpCountStore ipCountStore;

    private IpFailedLoginOverThresholdRule rule;

    @BeforeEach
    void setup() {
        rule = new IpFailedLoginOverThresholdRule(ipCountStore);
    ...
    void should_emit_event_for_ip_over_threshold() {
        ...
        when(ipCountStore.failedLoginByIp(ip)).thenReturn(OVER_THRESHOLD);
    ...
    void should_not_emit_event_for_ip_under_threshold() {
        final String ip = "127.0.0.1";
        when(ipCountStore.failedLoginByIp(ip)).thenReturn(UNDER_THRESHOLD);
